...
| Architecture | `stateless-mcp-incident-lab-architecture` | Scaffolded 2026-08-02 with 4 Proposed ADR stubs; all 4 are now `Status: Accepted`, covering independent raw/SDK realizations, DynamoDB state, Fargate/ALB transport, and ephemeral auth deferral, with `ARCH-001`–`ARCH-006` citations reserved for the active `/cdd-author` round |
...
| 3 | `transport/` | `http`, `sse` | POST endpoint, single-message bodies with 202 for notifications, Accept/content types, required mirrored headers, Base64 sentinel encoding, `x-mcp-header`, deterministic validation precedence, mismatch errors, bodiless Origin/GET/DELETE rejection with `Allow: POST` on 405, ignored `Last-Event-ID`, HTTP status and `Content-Type` mapping for every error envelope including 413/504 | 18 | 1,2 | High |
| 4 | `discovery/` | `http` | Mandatory `server/discover`, exact advertised capability set with no unimplemented capability claimed, identity, instructions, cache hints, direct-call-without-discovery | 6 | 1–3 | Medium |
...
| 7 | `mrtr/` | `tool-call` | `input_required`, form-mode capability check with no undeclared mode emitted, flat primitive `requestedSchema`, new retry ID, exact state echo, incomplete-response re-elicitation, accept/decline/cancel, signed state tamper/expiry/binding, cross-replica retry | 16 | 2,5,6 | Critical |
...
| 9 | `cache/` | `function`, `http` | Mandatory hints on the six cacheable operations and on no others, key method+params+cursor, TTL freshness, public/private scope, no MRTR caching, stale-on-error warning, consistent full-list rewalk, deterministic pages | 10 | 4,5,7 | Medium |
...
| 13 | `security/` | `http`, `lint-assertion` | Origin rebinding defense, malformed/unbounded schemas, header injection, bounded request body/deadline enforcement with no partial effect, bearer-handle entropy/unguessability floor, no concrete incident URI or bearer handle enumerable from any catalog result, output/state redaction, simulated-only actions | 12 | 3,5–8 | Critical |
| 14 | `observability/` | `http`, `trace-span` | Non-MCP `/raw/healthz` and `/sdk/healthz` readiness, structured logs, W3C trace context in `_meta`, method/name/replica/result metrics, bearer-handle/header/body redaction, other sensitive-field absence | 7 | 3,6 | Medium |
...
| 17 | `infra/` | `function` | CDK/Nginx assertions for encryption, TTL, least-privilege IAM/SG, ≥2 tasks, health checks, mandatory HTTPS with no public plaintext listener, WAF rate rule, bearer-bearing header/body absence from Nginx and ALB/WAF access logs, MRTR signing secret sourced from Secrets Manager and absent from plaintext task definitions, tags, log retention, scripted deploy→verify→destroy ordering, destroyability | 10 | 3,13–15 | High |
...
- `subscriptions/listen`, list-change notifications, resource subscriptions, and `completion/complete` argument autocompletion.
