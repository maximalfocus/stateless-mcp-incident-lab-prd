...
Every list operation (`tools/list`, `prompts/list`, `resources/list`, `resources/templates/list`) may be paginated. Page size is the server's choice, `nextCursor` is opaque to the client, the client follows cursors to completion without inferring meaning from their value, the cursor is part of the client cache key, and every page of one list carries the same `cacheScope` while each page may carry its own `ttlMs`. An empty string remains a valid opaque cursor and is not treated as end-of-results; only an absent `nextCursor` ends the walk. An invalid cursor returns `-32602`. The CLI requires a consistent full-list snapshot: if any cached page is stale, or a cursor is rejected mid-walk, it discards every cached page of that list and re-walks from the first page rather than mixing page generations or returning a partial list.
